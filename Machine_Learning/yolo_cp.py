...
def video_inference():
    ...
    video_path = "comp24_colored_cones_1.mp4"
    ...
    occupancy_grid = np.zeros((image_width,image_height),dtype=np.int32)
        ...
        if success: 
            ...
            if results[0].masks is not None:
                    if(len(results[0].masks.xy) != 0):
                        ...
                        segmasks = cv2.fillPoly(occupancy_grid,segment_array,[255,0,0])
                        ...
                        cv2.imshow("occupancy_grid",occupancy_grid)
                        cv2.waitKey(1)
